...
public class EnlightenmentCenter extends Robot {
    ...
    static Set<ComparableLocation> edgeMapLocations;
    static TreeSet<ComparableLocation> enemyMapLocations;
    static boolean enemyECTrapped = false;

    class ComparableLocation implements Comparable<ComparableLocation> {
        MapLocation ecLoc;
        MapLocation otherLoc;

        public ComparableLocation(MapLocation ecLoc, MapLocation edgeLoc) {
            this.otherLoc = edgeLoc;
            this.ecLoc = ecLoc;
        }

        @Override
        public int compareTo(ComparableLocation o) {
            return ecLoc.distanceSquaredTo(otherLoc) < o.ecLoc.distanceSquaredTo(o.otherLoc) ? -1 : 1;
        }

        @Override
        public boolean equals(Object o) {
            if (!(o instanceof ComparableLocation)) {
                return false;
            }

            ComparableLocation other = (ComparableLocation) o;
            return otherLoc.equals(other.otherLoc);
        }

        @Override public int hashCode() {
            return otherLoc.x * 31 + otherLoc.y * 17;
        }
    }
    ...
    public void run() throws GameActionException {
        ...
        System.out.println("TS " + enemyMapLocations.size());
    ...
    public void spawnRobot() throws GameActionException {
        ...
        int nearbyPoliticianCount = 0;
        int slanderers = 0;

        for (RobotInfo nearbyRobot : nearbyRobots) {
            if (nearbyRobot.team.equals(myTeam)) {
                if (nearbyRobot.type == RobotType.POLITICIAN) {
                    nearbyPoliticianCount++;
                } else if (nearbyRobot.type == RobotType.SLANDERER) {
                    slanderers++;
                }
            }
        }
        ...
        if (nearbyPoliticianCount < 8) {
            buildRobot(RobotType.POLITICIAN, randomDirection(), POLITICIAN_COST);
            ++numPoliticians;
        }

        if (slanderers < 4) {
            tryBuildInEveryDirection(RobotType.SLANDERER, SLANDERER_COST);
        }
            ...
            if (rc.canGetFlag(bot.ID)) {
                ...
                if (decodedFlag != null && decodedFlag.id != Constants.NOTHING) {
                    ...
                    if (decodedFlag.id == Constants.EDGE) {
                        ...
                        edgeMapLocations.add(new ComparableLocation(myCurrentLoc, decodedLocation));
                        ...
                        enemyMapLocations.add(new ComparableLocation(myCurrentLoc, decodedLocation));
                        messages[messageIndex] = comms.getFlag(decodedFlag.id, enemyMapLocations.first().otherLoc);
                        comms.setFlag(decodedFlag.id, enemyMapLocations.first().otherLoc);
                    }
...
//        checkIfEnemyECIsSurrounded();
    ...
    void bid() throws GameActionException {
        ...
        int curInfluence=rc.getInfluence();
        if (roundNum>1000) { //when round 1000, there is nothing more to change, just bid it all out
            percentage=0.2;
        }
        else if (roundNum>600 && curInfluence>100){ //start to rise bid pct after 600 round
            percentage = 0.1;
        }
